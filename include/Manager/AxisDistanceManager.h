...
public:
	...
	bool IsContain(ViewRange* actor); //计算制定AABB是包含自己
	...
	const b2AABB& GetBodyAABB(PointType type) const;//获取到当前角色的刚体AABB  
	const std::list<b2AABB*>& GetSplitAABB(PointType type);//获取到当前的裁剪AABB 
	void RecalcAABBRange(PointType type);//重新计算视口 
	void RecalcBodyAABB(PointType type);//重新计算视口
	void RecalcRefreshCondtion();//重计算刷新条件  
	void CalcSplitAABB(PointType type);//计算分割刚体(AABB会经常算,视图不经常算)
	void RecycleAABBPoint(PointType type);//回收点位 
	~ViewRange(); 
private:
...
public:
	...
	void ABBBRnageRecycle(float pos, ViewRangeRecordMap& axisMap);//回收AABB单元
	//获取到单元
...
inline const b2AABB& ViewRange::GetBodyAABB(PointType type) const//获取到当前角色的刚体AABB 
{
	return m_BodyAABB[type];//获取到某一类型的AABB盒子
}
inline const std::list<b2AABB*>& ViewRange::GetSplitAABB(PointType type)//获取到当前的裁剪AABB
{
	return m_SplitAABBList[type];
}

inline void ViewRange::RecalcBodyAABB(PointType type)//重新计算视口
{
...
inline void ViewRange::RecycleAABBPoint(PointType type)//回收点位
{
