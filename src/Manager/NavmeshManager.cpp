...
using std::vector;
using std::string;
NavmeshID NavmeshManager::AllocNavmeshID = 1;
NavmeshManager::NavmeshManager() {
}  
//生成加载一个navmesh
NavmeshID NavmeshManager::GenerateNavmesh(string fileName)//返回一个meshID,后期用这个ID进行寻路地图的维护
{
...
//删除一个地图
void NavmeshManager::RemoveNavmesh(NavmeshID navID)
{
	...
	//加入一个角色
bool NavmeshManager::RegisterActor(NavmeshID navID, ActorID actorId)
{
...
void NavmeshManager::UnregisterActor(ActorID actorId)
{
	if (MapInner.find(actorId) == MapInner.end())//如果当前已经加入了地图
		return; 
...
//寻路
void NavmeshManager::FindPath(int64_t actorID,b2Vec2 startPos,b2Vec2 endPos,bool soomth )//开始位置 结束位置,是否平滑路径
...
//寻找一个随机寻路点
bool NavmeshManager::FindRandPoint(NavmeshID navID, PosV3& pos)
{
	if (NavmeshMap.find(navID) == NavmeshMap.end())
		return false;
...
//射线查询
float NavmeshManager::Recast(ActorID actorID, b2Vec2 startPos, b2Vec2 endPos)//开始位置 结束位置
{	//首先判断当前玩家是否有加入到navmesh中
	if (MapInner.find(actorID) == MapInner.end())//如果当前已经加入了地图
		return 0;
	PosV3 start(startPos.x, 0, startPos.y);
...
//分片寻路,好了之后会进行通知
void NavmeshManager::SlicedFindPath(ActorID actorID, b2Vec2 startPos, b2Vec2 endPos)
{	//首先判断当前玩家是否有加入到navmesh中
	if (MapInner.find(actorID) == MapInner.end())//如果当前已经加入了地图
		return;
	PosV3 start(startPos.x, 0, startPos.y);
