...
struct CDelaBella2 : IDelaBella2<T,I>
{
	...
	{
		uint64_t time0 = uSec();//首先获取到当前的时间

		if (errlog_proc)
			errlog_proc(errlog_file, "[...] sorting vertices");//打印输出排序的顶点
		I points = inp_verts;

		std::sort(vert_alloc, vert_alloc + points);//对顶点进行排序 排序规则没看懂

		// rmove dups
		{
			vert_map[vert_alloc[0].i] = 0;//顶点数组 执行的所有设置为0

			I w = 0, r = 1; // skip initial no-dups block
			while (r < points && !Vert::overlap(vert_alloc + r, vert_alloc + w))//循环遍历判断是否存在点覆盖 两点胸痛的话
			{
				vert_map[vert_alloc[r].i] = r;//设置当前顶点等于
				w++;
				r++;
			}
	...
	I Triangulate(I* other_faces)
	{
		I i = 0;//index索引
		Face* hull = 0;//外壳面
		I hull_faces = 0;//外壳面数目
		Face* cache = 0;//零食cache

		uint64_t sort_stamp;//排序用
		...
		for (; i < points; i++)//循环遍历所有的点
		{
			...
			while (stack)
			{
				...
				Vert* fv[3] = { (Vert*)f->v[0],(Vert*)f->v[1],(Vert*)f->v[2] };//关联的三个点
				Face* ff[3] = { (Face*)f->f[0],(Face*)f->f[1],(Face*)f->f[2] };//关联的三个面
	...
	{
		inp_verts = points;//输入的节点信息
		out_verts = 0;//输出的节点信息
		polygons = 0;//多边形

		first_dela_face = 0;//第一个三角形的面
		first_hull_face = 0;//第一个面的面
		first_boundary_vert = 0;//第一个分界线图形

		if (max_verts < points)//如果最大的点 小雨当前的点
		{
			if (max_verts)//如果最大点为真的话
			{
				free(vert_map);//删除顶点指针
				vert_map = 0;

				free(vert_alloc);//删除顶点内存分配
				//delete [] vert_alloc;
				vert_alloc = 0;
				max_verts = 0;//重置数据
			}
			...
			*/
			...
			vert_alloc = (Vert*)malloc(sizeof(Vert) * points);//顶点内存分配

			if (vert_alloc)
				vert_map = (I*)malloc(sizeof(I) * (size_t)points);//顶点映射分配

			if (vert_alloc && vert_map)
				max_verts = points;//设置最大点信息
			else
			{
				if (errlog_proc)
					errlog_proc(errlog_file, "[ERR] Not enough memory, shop for some more RAM. See you!\n");
	...
	virtual I ConstrainEdges(I edges, const I* pa, const I* pb, size_t advance_bytes)
	{
		...
		int pro = 0;
		for (I con = 0; con < edges; con++)//循环遍历6条边
		{
			if (con >= pro)
			{
			...
			I a = *(const I*)((const char*)pa + (intptr_t)con * advance_bytes);//获取到a的index
			I b = *(const I*)((const char*)pb + (intptr_t)con * advance_bytes);//获取到b的index

			if (!first_dela_face || a == b)//如果第一个面不存在 或者 两点相同的haunt
				continue;

			// current
			Vert* va = (Vert*)GetVertexByIndex(a);//找到这个 带你
			if (!va)
				continue;

			// destination
			Vert* vc = (Vert*)GetVertexByIndex(b);//找到另一个点
			if (!vc)
				continue;
			...
			Face** tail = 0;
			Vert* restart = 0;
			Face* list = FindConstraintOffenders(va, vc, &tail, &restart);//寻找重叠的面
	...
	virtual I Triangulate(I points, const T* x, const T* y, size_t advance_bytes)
	{
		...
		if (advance_bytes < sizeof(T) * 2)//每一次步进的单位
			advance_bytes = sizeof(T) * 2;

		if (!ReallocVerts(points))//开始重新分配点数目
			return 0;

		for (I i = 0; i < points; i++)//循环遍历所有的点
		{
			Vert* v = vert_alloc + i;//获取到顶点数组分配
			v->i = i;//分配index
			v->x = *(const T*)((const char*)x + i*advance_bytes);//设置其X
			v->y = *(const T*)((const char*)y + i*advance_bytes);//设置其Y
		}

		out_hull_faces = 0;//输出所有的面 设置为0 
		unique_points = 0;//特殊点
		out_verts = Triangulate(&out_hull_faces);//输出的三角形顶点
		polygons = out_verts / 3;
		return out_verts;
	}
